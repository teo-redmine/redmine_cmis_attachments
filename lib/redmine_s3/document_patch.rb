...
#          cmisDocuments = RedmineS3::Connection.get_documents_in_folder(cmis_object_id)
...
#      end
      ...
      def cmis_object_id
        if self.id
          ...
          folder = RedmineS3::Connection.folder_by_tree_and_name(project.cmis_object_id, DocumentCategory.find(document.category_id).name)
          if !folder.nil?
